...
class TeachError {
  ...
  TeachError.fromParams(
      {this.code, this.count, this.msg, this.time, this.data});

  factory TeachError(jsonStr) => jsonStr == null
      ? null
      : jsonStr is String
          ? new TeachError.fromJson(json.decode(jsonStr))
          : new TeachError.fromJson(jsonStr);

  TeachError.fromJson(jsonRes) {
    code = jsonRes['code'];
    count = jsonRes['count'];
    msg = jsonRes['msg'];
    time = jsonRes['time'];
...
class Data {
  ...
  Data.fromJson(jsonRes) {
    ...
    teach_exam_id = jsonRes['teach_exam_id'];
    user_id = jsonRes['user_id'];
    answer = jsonRes['answer'];
    ctime = jsonRes['ctime'];
    error_time = jsonRes['error_time'];
    mtime = jsonRes['mtime'];
    classify = jsonRes['classify'] == null
        ? null
        : new Classify.fromJson(jsonRes['classify']);
    teachexam = jsonRes['teachexam'] == null
        ? null
        : new TeachExam.fromJson(jsonRes['teachexam']);
    user = jsonRes['user'] == null ? null : new User.fromJson(jsonRes['user']);
  }

  @override
  String toString() {
    return '{"id": $id,"status": $status,"teach_exam_id": $teach_exam_id,"user_id": $user_id,"answer": ${answer != null ? '${json.encode(answer)}' : 'null'},"ctime": ${ctime != null ? '${json.encode(ctime)}' : 'null'},"error_time": ${error_time != null ? '${json.encode(error_time)}' : 'null'},"mtime": ${mtime != null ? '${json.encode(mtime)}' : 'null'},"classify": $classify,"teachexam": $teachexam,"user": $user}';
  }
}
...
class TeachExam {
  ...
  @override
  String toString() {
    return '{"classify_id": $classify_id,"id": $id,"option_key1": $option_key1,"option_key2": $option_key2,"option_key3": $option_key3,"option_key4": $option_key4,"option_key5": $option_key5,"score": $score,"status": $status,"type": $type,"user_id": $user_id,"ctime": ${ctime != null ? '${json.encode(ctime)}' : 'null'},"keyword": ${keyword != null ? '${json.encode(keyword)}' : 'null'},"mtime": ${mtime != null ? '${json.encode(mtime)}' : 'null'},"option_value1": ${option_value1 != null ? '${json.encode(option_value1)}' : 'null'},"option_value2": ${option_value2 != null ? '${json.encode(option_value2)}' : 'null'},"option_value3": ${option_value3 != null ? '${json.encode(option_value3)}' : 'null'},"option_value4": ${option_value4 != null ? '${json.encode(option_value4)}' : 'null'},"option_value5": ${option_value5 != null ? '${json.encode(option_value5)}' : 'null'},"topic": ${topic != null ? '${json.encode(topic)}' : 'null'}}';
  }
}
